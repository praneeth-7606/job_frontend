...
function Jobtype() {
    ...
    const addJobType = async () => {
        if (newJobType.trim() !== '') {
            try {
                await axios.post('http://localhost:3002/api/route/jobtypes/createjobtype', { name: newJobType });
                setNewJobType('');
                fetchJobTypes();
            } catch (error) {
    ...
    const handleSave = async (index, updatedValue, id) => {
        try {
            await axios.put(`http://localhost:3002/api/route/jobtypes/update-jobtype/${id}`, { name: updatedValue });
            fetchJobTypes();
            setEditIndex(null);
        } catch (error) {
